...
export default function ThemeBackground({ themeColor, isDark }: ThemeBackgroundProps) {
  return (
    <div
      className={`fixed inset-0 z-0 overflow-hidden transition-colors duration-1000 ${isDark ? 'bg-[#020202]' : 'bg-[#fcfcfc]'}`}
    >
      {/* Breathing Blob */}
      <motion.div
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at center, transparent 30%, ${isDark ? 'rgba(0,0,0,0.95)' : 'rgba(255,255,255,0.95)'} 100%)`,
        }}
      />
    </div>
  );

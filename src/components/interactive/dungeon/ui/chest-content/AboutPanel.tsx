  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
        ...
        <p className="mt-3 text-sm leading-relaxed text-stone-300">{PORTFOLIO_CONTENT.hero.tagline}</p>
      </div>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {PORTFOLIO_CONTENT.about.stats.map((stat, index) => {
      ...
      <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-white/[0.05] to-transparent p-4">
        <p className="text-sm text-stone-300 leading-relaxed">{PORTFOLIO_CONTENT.about.description}</p>
      </div>
    </div>
  );
}

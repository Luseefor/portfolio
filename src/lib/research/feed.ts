...
async function fetchSource(source: ResearchSourceConfig): Promise<{
  items: ResearchItem[];
    ...
    if (!envKeyName || !token) {
      return {
        ...
        failure: createFailure(
          source,
          'SOURCE_AUTH',
          'Missing authEnvKey or environment token value for source authentication.',
        ),
      };
    }
  ...
  try {
    const requestUrl =
      source.method === 'GET' ? buildUrlWithQuery(source.endpoint, source.query) : source.endpoint;
    ...
    if (!response.ok) {
      return { items: [], failure: mapHttpFailure(source, response.status) };
    }
    ...
    try {
      ...
      return {
        items: [],
        failure: createFailure(source, 'SOURCE_SCHEMA', 'Source response was not valid JSON.'),
      };
    }
    ...
    try {
      ...
      return {
        items: [],
        failure: createFailure(source, 'SOURCE_SCHEMA', 'Failed to map source payload to feed items.'),
      };
    }
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') {
      return {
        items: [],
        failure: createFailure(source, 'SOURCE_TIMEOUT', `Source timed out after ${source.timeoutMs}ms.`),
      };
    }

    return {
      items: [],
      failure: createFailure(source, 'SOURCE_NETWORK', 'Network request to source failed.'),
    };
  } finally {
